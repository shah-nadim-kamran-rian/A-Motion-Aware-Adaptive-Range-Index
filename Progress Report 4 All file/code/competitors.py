"""
competitors.py -- real ordered-index competitors for MARI on the exact
range-reporting-under-drift problem. Each competitor exposes the same interface
(insert, update, delete, range) and instrumentation counters, and is checked
exact against the brute-force oracle.

  ART       : Adaptive Radix Tree (Node4/16/48/256), the canonical cache-friendly
              ordered index. Under drift every value change is a relocation
              (delete old key + insert new key).
  PGMIndex  : dynamic PGM-style learned index (piecewise-linear segments over the
              sorted key array, rebuilt by the logarithmic method).
  BxExact   : Bx-tree with time partitions + an EXACTNESS ADAPTER (query expands
              over partitions, candidates verified against current value).

Instrumentation (implementation-independent):
  relocations  -- structural key moves caused by value changes
  node_writes  -- node allocations / type upgrades (ART) or segment rebuild work
  scanned      -- candidate entries examined by a query
  verifies     -- authoritative-value checks performed by a query
"""
...
class _Run:
    ...
    def lower_bound(self, key):
        ...
        if not self.vals:
            return 0
        p = self._predict(key)
        n = len(self.vals); lo = min(n, max(0, p - self.eps - 1)); hi = min(n, max(0, p + self.eps + 1))
